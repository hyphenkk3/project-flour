import type { Metadata } from "next";
import { redirect } from "next/navigation";
import { StaffForcedPasswordChangeForm } from "@/components/settings/StaffForcedPasswordChangeForm";
import { logoutAction } from "@/foundation/auth/actions";
import { resolvePostLoginDestination } from "@/foundation/auth/post-login-destination";
import { requireStaff } from "@/foundation/auth/session";
import { STAFF_FORCED_PASSWORD_COPY } from "@/foundation/staff/forced-password-change";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Create a new password · Whitebird Operating System",
};

export default async function CompletePasswordChangePage() {
  const staff = await requireStaff();

  if (!staff.mustChangePassword) {
    redirect(resolvePostLoginDestination(staff.role.code));
  }

  return (
    <main className="relative flex min-h-dvh flex-col justify-center px-6 py-16 sm:px-10">
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_12%_8%,#ffffff_0%,transparent_42%),linear-gradient(160deg,#eef2f5_0%,#e3ebf1_52%,#d5e1ea_100%)]"
      />
      <div className="relative mx-auto w-full max-w-md">
        <p className="text-signal text-xs font-medium tracking-[0.22em] uppercase">
          Whitebird Operating System
        </p>
        <h1 className="font-display text-ink mt-4 text-4xl tracking-tight sm:text-5xl">
          {STAFF_FORCED_PASSWORD_COPY.heading}
        </h1>
        <p className="text-skyline mt-3">
          {STAFF_FORCED_PASSWORD_COPY.explanation}
        </p>
        <StaffForcedPasswordChangeForm />
        <form action={logoutAction} className="mt-6">
          <button
            className="text-skyline hover:text-ink text-sm font-medium"
            type="submit"
          >
            {STAFF_FORCED_PASSWORD_COPY.signOut}
          </button>
        </form>
      </div>
    </main>
  );
}
