...
export function BakeryOrderDetail({ order, boardDate }: BakeryOrderDetailProps) {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-5 pt-5 pb-16 sm:px-8 sm:pt-8">
      <Link
      ...
      {!secured ? (
        <p className="border-status-warning/20 bg-status-warning-soft text-status-warning mt-6 rounded-2xl border px-4 py-3 text-sm leading-relaxed">
          Not secured — this preorder is still being confirmed. Please check
          the order status before starting production.
        </p>
      ) : null}
      ...
      {paymentAttention ? (
        <p className="border-status-danger/20 bg-status-danger-soft text-status-danger mt-6 rounded-2xl border px-4 py-3 text-sm leading-relaxed">
          Payment Attention — balance needs Owner resolution. Bakery keeps this
          order visible because it is already Ready. Do not chase payment from
          Bakery.
        </p>
      ) : null}
