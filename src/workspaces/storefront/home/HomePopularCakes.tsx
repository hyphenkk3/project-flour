...
export function HomePopularCakes({ cakes }: HomePopularCakesProps) {
  return (
    <section className="border-ink/[0.1] mt-8 min-w-0 border-t px-6 pt-8 sm:px-10">
              ...
              {cakes.map((cake) => {
                ...
                return (
                  <li className="w-24 shrink-0 lg:w-[10.5rem]" key={cake.id}>
                    <Link className="group block" href={`/cakes/${cake.id}`} prefetch>
                      <div className="relative aspect-square h-24 w-24 overflow-hidden rounded-[10px] lg:h-[10.5rem] lg:w-[10.5rem]">
                        {photo?.url ? (
                          <CakePhotoImage
                            ...
                            sizes="(min-width: 1024px) 168px, 96px"
                            src={photo.url}
                          />
                        ) : (
