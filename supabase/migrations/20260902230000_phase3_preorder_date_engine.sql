-- Phase 3 — Preorder / date engine (customer Whole Cake)
-- Replaces Singapore today+2 in submit_guest_preorder with:
--   malaysia_preorder_business_date() + max(library_cake_sizes.preorder_days)
-- Adds a capacity hook (Fully Booked) that never exposes quantities.
-- Extra / Fresh Picks submit_guest_extra_order is unchanged.
-- Does not rewrite Phase 2 schema.

create or replace function public.earliest_preorder_collection_date(
  p_preorder_days integer,
...
as $$
begin
  if p_preorder_days is null or p_preorder_days < 1 then
    raise exception 'preorder_days must be at least 1';
  end if;
  return public.malaysia_preorder_business_date(p_ts) + p_preorder_days;
end;
$$;

comment on function public.earliest_preorder_collection_date(integer, timestamptz) is
  'DAY 0 (Malaysia preorder business date) plus per-size preorder_days. '
  'Does not apply catalogue windows, closures, or capacity.';

revoke all on function public.earliest_preorder_collection_date(integer, timestamptz)
  from public;
grant execute on function public.earliest_preorder_collection_date(integer, timestamptz)
  to anon, authenticated;

-- Internal: true when a matching production_capacity row is exceeded.
-- No matching row = unrestricted (not fully booked).
create or replace function public._guest_preorder_item_fully_booked(
  p_pickup_date date,
...
as $$
declare
  v_capacity public.production_capacity%rowtype;
  v_used integer;
begin
  ...
  return (v_used + p_quantity) > v_capacity.capacity_quantity;
end;
$$;

revoke all on function public._guest_preorder_item_fully_booked(
  date, uuid, uuid, uuid, integer
...
begin
  ...
  if exists (
    ...
    where (item ->> 'cake_id') is null
       or (item ->> 'cake_size_id') is null
       or not exists (
         select 1
         from public.library_cake_sizes s
         where s.id = (item ->> 'cake_size_id')::uuid
           and s.cake_id = (item ->> 'cake_id')::uuid
       )
  ) then
  ...
  join public.library_cake_sizes s
    on s.id = (item ->> 'cake_size_id')::uuid
   and s.cake_id = (item ->> 'cake_id')::uuid;
