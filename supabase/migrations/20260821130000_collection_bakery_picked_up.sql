-- Bakery shared access to Collection handoff.
--
-- Allows Bakery staff to mark/undo guest Pickup orders as Picked Up.
-- No other Collection permissions are changed.
--
-- Allowed: owner | manager | collection | customer_operations | bakery
-- Denied: all other roles
--
-- This preserves the existing RPC behavior:
-- - Pickup only; Delivery remains refused
-- - Existing order status rules remain unchanged
-- - Ready is not required at SQL level
...
comment on function public.mark_guest_order_picked_up(uuid, uuid) is
  'Mark guest Pickup order Picked Up. Roles owner|manager|collection|customer_operations|bakery. Ready not required at SQL. Delivery refused.';
...
comment on function public.undo_guest_order_picked_up(uuid, uuid) is
  'Undo guest Pickup Picked Up. Roles owner|manager|collection|customer_operations|bakery. Preserves Ready. Delivery refused.';

-- Preserve the existing execution grants.
