-- Customer Whole Cake preorder options.
-- Website guests may select complimentary items and Birthday/Wishing Cards.
-- Order cutoff / catalogue / pickup rules are unchanged.
-- Confirmation format is unchanged: persist canonical complimentary + paid-add-on rows
-- so existing confirmation/crew rendering can produce the current output.

-- ---------------------------------------------------------------------------
-- 1) Storefront option catalog (anon-readable via security definer)
-- ---------------------------------------------------------------------------
...
comment on function public.storefront_customer_preorder_options(uuid) is
  'Public Whole Cake options: available complimentary Birthday Topper / Candle / Knife '
  'plus active Birthday Card / Wishing Card catalog prices. Does not expose inactive types.';
...
-- ---------------------------------------------------------------------------
-- 2) submit_guest_preorder — persist customer selections (defaults empty)
-- ---------------------------------------------------------------------------
...
begin
  ...
  if p_paid_addons is not null
    ...
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          'code', trim(addon ->> 'code'),
          'quantity', 1,
          'messages', case
            when addon ? 'messages' and jsonb_typeof(addon -> 'messages') = 'array'
              then jsonb_build_array(addon -> 'messages' -> 0)
            when addon ? 'written_message'
              then jsonb_build_array(addon -> 'written_message')
            else '[]'::jsonb
          end
        )
        order by trim(addon ->> 'code')
      ),
      '[]'::jsonb
    )
    into v_paid
...
comment on function public.submit_guest_preorder(
  text, text, text, date, time, text, jsonb, boolean, jsonb, jsonb
) is
  'Website Whole Cake preorder. Cake catalogue, pickup, and cutoff rules unchanged. '
  'p_complimentary and p_paid_addons persist customer selections for existing confirmation. '
  'Empty arrays mean none selected (no automatic complimentary defaults). '
  'Paid-add-on prices snapshot from paid_addon_types.';
