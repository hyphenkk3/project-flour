-- Guest preorder Delivery processing-fee acknowledgement.
-- Delivery website checkout must send p_delivery_processing_fee_ack matching
-- current_delivery_processing_fee_default() (RM5). Pickup / dine-in do not
-- require it. Fresh Picks / EXTRA and cake-size price ack are unchanged.
...
declare
  ...
  v_applicable numeric(10, 2);
  v_quoted numeric(10, 2);
  v_acked numeric(10, 2);
  v_ack_line jsonb;
  v_ack_date text;
  v_price_ack_stale text :=
...
begin
  ...
  loop
    ...
    v_applicable := public.library_cake_size_price_on(size_row.id, p_pickup_date);

    if p_price_ack is not null and jsonb_typeof(p_price_ack) = 'object' then
      v_ack_date := nullif(trim(coalesce(p_price_ack ->> 'pickup_date', '')), '');
      if v_ack_date is null or v_ack_date::date is distinct from p_pickup_date then
        raise exception '%', v_price_ack_stale;
      end if;

      v_ack_line := null;
      if jsonb_typeof(p_price_ack -> 'lines') = 'array' then
        select line
        into v_ack_line
        from jsonb_array_elements(p_price_ack -> 'lines') as line
        where (line ->> 'cake_size_id') = v_size_id::text
        limit 1;
      end if;

      if v_ack_line is null then
        raise exception '%', v_price_ack_stale;
      end if;

      begin
        v_quoted := (v_ack_line ->> 'quoted_unit_price')::numeric;
        v_acked := (v_ack_line ->> 'acknowledged_unit_price')::numeric;
      exception when others then
        raise exception '%', v_price_ack_stale;
      end;

      if round(v_quoted, 2) is distinct from round(v_applicable, 2) then
        if v_acked is null
           or round(v_acked, 2) is distinct from round(v_applicable, 2) then
          raise exception '%', v_price_ack_stale;
        end if;
      end if;
    end if;
...
comment on function public.submit_guest_preorder(
  text, text, text, date, time, text, jsonb, boolean, jsonb, jsonb, boolean,
  public.fulfilment_method, jsonb, jsonb, jsonb, jsonb
) is
  ...
  'Optional p_price_ack verifies quoted vs applicable cake-size prices. '
