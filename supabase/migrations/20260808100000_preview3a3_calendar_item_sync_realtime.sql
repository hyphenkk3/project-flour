-- Preview 3A-3: ensure order-item sync refreshes Calendar Cake View via orders Realtime.
-- sync_guest_order_items mutates only order_items; without touching orders, item-only
-- changes would not emit an orders UPDATE. Owner Save already updates orders first,
-- but this makes the RPC itself Calendar-safe.
-- Additive. Does not amend applied migrations.
...
begin
  ...
  -- Emit orders Realtime so Whole Cake Calendar Cake View refreshes item snapshots.
...
comment on function public.sync_guest_order_items(uuid, jsonb) is
  'Owner Order Workspace item sync. Allowed while submitted, pending_confirmation, awaiting_payment, or paid. Touches orders.updated_at so Calendar Cake View receives Realtime. Does not mutate payments.';
