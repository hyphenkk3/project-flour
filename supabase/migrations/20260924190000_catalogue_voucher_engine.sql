-- Catalogue voucher eligibility rules + apply RPC.
-- Separate from August Promo and RM10 physical cards.
-- Absent rules impose no restriction. Do not alter library_vouchers identity.

create type public.library_voucher_rule_type as enum (
  'order_date',
  'fulfilment_date',
...
create table public.library_voucher_rules (
  id uuid primary key default gen_random_uuid(),
  voucher_id uuid not null references public.library_vouchers (id) on delete cascade,
  rule_type public.library_voucher_rule_type not null,
  date_from date,
  date_until date,
  amount numeric(10, 2),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint library_voucher_rules_voucher_type_unique unique (voucher_id, rule_type),
  constraint library_voucher_rules_date_range check (
    date_from is null
    or date_until is null
    or date_until >= date_from
  ),
  constraint library_voucher_rules_amount_non_negative check (
    amount is null or amount >= 0
  )
);

create table public.library_voucher_rule_values (
  id uuid primary key default gen_random_uuid(),
  rule_id uuid not null references public.library_voucher_rules (id) on delete cascade,
  cake_id uuid references public.library_cakes (id) on delete cascade,
  cake_size_id uuid references public.library_cake_sizes (id) on delete cascade,
  size_label text,
  created_at timestamptz not null default now(),
  constraint library_voucher_rule_values_has_target check (
    cake_id is not null
...
create unique index library_voucher_rule_values_cake_unique
  on public.library_voucher_rule_values (rule_id, cake_id)
  where cake_id is not null;

create unique index library_voucher_rule_values_size_label_unique
  on public.library_voucher_rule_values (rule_id, lower(trim(size_label)))
  where size_label is not null;

create unique index library_voucher_rule_values_size_id_unique
  on public.library_voucher_rule_values (rule_id, cake_size_id)
  where cake_size_id is not null;

create index library_voucher_rules_voucher_idx
  on public.library_voucher_rules (voucher_id);

create trigger library_voucher_rules_set_updated_at
before update on public.library_voucher_rules
for each row
execute function public.set_updated_at();

alter table public.library_voucher_rules enable row level security;
alter table public.library_voucher_rule_values enable row level security;

create policy "Authenticated staff can select library voucher rules"
on public.library_voucher_rules for select to authenticated using (true);
create policy "Authenticated staff can insert library voucher rules"
on public.library_voucher_rules for insert to authenticated with check (true);
create policy "Authenticated staff can update library voucher rules"
on public.library_voucher_rules for update to authenticated using (true) with check (true);
create policy "Authenticated staff can delete library voucher rules"
on public.library_voucher_rules for delete to authenticated using (true);

create policy "Authenticated staff can select library voucher rule values"
on public.library_voucher_rule_values for select to authenticated using (true);
create policy "Authenticated staff can insert library voucher rule values"
on public.library_voucher_rule_values for insert to authenticated with check (true);
create policy "Authenticated staff can update library voucher rule values"
on public.library_voucher_rule_values for update to authenticated using (true) with check (true);
create policy "Authenticated staff can delete library voucher rule values"
on public.library_voucher_rule_values for delete to authenticated using (true);

create or replace function public.normalize_catalogue_size_label(p_label text)
returns text
language sql
immutable
as $$
  select nullif(
    lower(replace(replace(replace(trim(coalesce(p_label, '')), ' ', ''), '''', '"'), '″', '"')),
    ''
  );
$$;
