-- Skip no-op production_capacity_events when saved quantity equals existing quantity.
-- Does not change capacity formulas, confirmed-order floor, or remove semantics.
-- Reversible by re-applying public.set_production_capacity from 20260903100000.
...
begin
  ...
  -- Existing row with the same quantity is not a capacity change.
  -- Initial set (no previous row) still records an event.
  if v_has_row and v_previous is not distinct from p_capacity_quantity then
    return jsonb_build_object(
      'id', v_id,
      'capacity_quantity', p_capacity_quantity,
      'committed_quantity', v_committed
