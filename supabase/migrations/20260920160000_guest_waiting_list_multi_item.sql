-- Guest Waiting List: one request may contain multiple cake/size items.
-- Consolidates duplicate cake/size lines, validates guest catalogue membership,
-- and keeps the closed-date Fully Booked exemption from 20260920140000.
-- Open dates still require Fully Booked. Staff insert is unchanged.
-- Does not change staff notifications, event keys, or capacity semantics.
-- Does not cap requested quantity by production_capacity.quantity.
...
begin
  ...
  if exists (
    ...
    where nullif(trim(coalesce(value->>'cake_id', value->>'cakeId', '')), '') is null
       or nullif(trim(coalesce(value->>'cake_size_id', value->>'sizeId', '')), '') is null
  ) then
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'cake_id', cake_id,
        'cake_size_id', size_id,
        'quantity', quantity
      )
      order by cake_id, size_id
    ),
    '[]'::jsonb
  )
  into v_items
  from (
    ...
    from (
      select
        nullif(trim(coalesce(value->>'cake_id', value->>'cakeId', '')), '')::uuid as cake_id,
        nullif(trim(coalesce(value->>'cake_size_id', value->>'sizeId', '')), '')::uuid as size_id,
  ...
  loop
    ...
    if p_actor_staff_id is null then
      if not exists (
        ...
        raise exception 'One of the selected cakes is no longer available on the waiting list for that date';
      end if;
      if not exists (
        select 1
        from public.collection_cakes cc
        where cc.collection_id = v_collection.id
          and cc.library_cake_id = v_cake
          and cc.available is true
      ) then
        raise exception 'One of the selected cakes is no longer available on the waiting list for that date';
      end if;
    end if;
    ...
    -- Requested quantity is not capped by production_capacity.quantity.
