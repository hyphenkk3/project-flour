-- Treat library-managed physical_discount_vouchers.expiry_date as the
-- authoritative original expiry during RM10 redemption.
-- Form expiry remains verification metadata and must not make an expired
-- registered card valid. Owner Override is still required explicitly.
-- Unregistered/historical vouchers keep form-expiry eligibility.
-- This migration does not change the preserve-expiry write from
-- 20260924170000_preserve_library_rm10_expiry.sql.

CREATE OR REPLACE FUNCTION public.redeem_rm10_physical_voucher_for_guest_order(p_order_id uuid, p_actor_staff_id uuid, p_voucher_number text, p_expiry_date date, p_owner_override boolean DEFAULT false, p_override_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  if v_found then
    ...
    -- Form expiry is verification metadata only and cannot make an expired
    -- registered card valid.
    if voucher_row.library_managed
      and voucher_row.expiry_date is not null then
      v_valid := true;
      v_invalid_reason := null;
      if v_order_date > voucher_row.expiry_date then
        v_valid := false;
        v_invalid_reason := 'Order date is after voucher expiry';
      elsif v_pickup > voucher_row.expiry_date then
        v_valid := false;
        v_invalid_reason := 'Pickup date is after voucher expiry';
      elsif public.singapore_calendar_date(now()) > voucher_row.expiry_date then
        v_valid := false;
        v_invalid_reason := 'This physical voucher has expired';
      end if;
    end if;
  end if;
  ...
  insert into public.order_adjustments (
    ...
    jsonb_build_object(
      ...
      'registered_expiry_date', case
        when voucher_row.library_managed then to_char(voucher_row.expiry_date, 'YYYY-MM-DD')
        else null
      end
    ),
    p_actor_staff_id
...
CREATE OR REPLACE FUNCTION public.change_august_promo_to_rm10_physical_voucher(p_order_id uuid, p_actor_staff_id uuid, p_voucher_number text, p_expiry_date date, p_owner_override boolean DEFAULT false, p_override_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
