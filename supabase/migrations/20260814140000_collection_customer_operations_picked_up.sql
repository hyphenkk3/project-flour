-- Shared Operations access — Collection handoff for Customer Operations.
-- Narrow allowlist only: add customer_operations to existing Picked Up RPCs.
-- Allowed: owner | manager | collection | customer_operations
-- Denied: bakery | other (unchanged)
-- No lifecycle, column, Ready-rule, or Delivery semantics changes.
...
begin
  ...
  if v_role is null
    or v_role not in ('owner', 'manager', 'collection', 'customer_operations')
  then
...
comment on function public.mark_guest_order_picked_up(uuid, uuid) is
  'Mark guest Pickup order Picked Up. Roles owner|manager|collection|customer_operations. Ready not required at SQL. Delivery refused.';
...
comment on function public.undo_guest_order_picked_up(uuid, uuid) is
  'Undo guest Pickup Picked Up. Roles owner|manager|collection|customer_operations. Preserves Ready. Delivery refused.';
