...
-- Owner Edit Order → Save Changes left old order_items rows in place because
-- public.order_items had SELECT/INSERT/UPDATE RLS policies but NO DELETE policy.
-- PostgREST delete therefore matched 0 rows (no error), then insert added a new row.
--
-- This migration:
-- 1) Adds authenticated DELETE on order_items
-- 2) Adds transactional sync_guest_order_items RPC for owner save path
-- No order data is modified.

-- ---------------------------------------------------------------------------
-- 1) Allow authenticated staff to delete order_items
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2) Transactional replace of guest order line items
-- ---------------------------------------------------------------------------
...
begin
  ...
  if order_row.status not in ('submitted', 'pending_confirmation') then
