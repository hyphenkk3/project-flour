/**
 * Live disposable verification after 20260818130000 addon-ambiguity fix.
 * Cleanup always. Does not mutate Product order 7e9779ac-….
 *
 * Run: npx tsx scripts/test-guest-preorder-addon-ambiguity-live.ts
 */
...
async function main() {
  ...
  try {
    ...
    type Size = { id: string; label: string; price: number | string };
    type Cake = {
      id: string;
      name: string;
      status: string;
    ...
    async function submit(input: {
      ...
      const { data, error } = await admin.rpc("submit_guest_preorder", {
        ...
        p_phone: PHONE,
        p_email: input.emailReceipt
          ? "wb-fix-addon-20260818@example.test"
          : null,
        p_pickup_date: PICKUP_DATE,
    ...
    async function loadOrder(orderId: string) {
      ...
      const { data: comps } = await admin
        ...
        .eq("order_id", orderId);
      return { order, orderItems: orderItems ?? [], addons: addons ?? [], comps: comps ?? [] };
    }
    ...
    function confirmationFor(loaded: Awaited<ReturnType<typeof loadOrder>>) {
      ...
      const paidAddons = loaded.addons.map((row, index) => {
        const child = [...((row.order_paid_addon_messages as Array<{
          card_index: number;
          written_message: string | null;
        }> | null) ?? [])].sort((a, b) => a.card_index - b.card_index);
        return {
    ...
    const cakeOnly = await submit({
      suffix: "CakeOnly",
      complimentary: [],
      paidAddons: [],
      emailReceipt: true,
    });
    check(cakeOnly.error == null, "A cake-only submit", cakeOnly.error ?? undefined);
    if (cakeOnly.id) {
      const loaded = await loadOrder(cakeOnly.id);
      const conf = confirmationFor(loaded);
      check(conf.amountDue === 135, "A total RM135", String(conf.amountDue));
      check(loaded.addons.length === 0, "A no paid-card rows");
      check(loaded.comps.length === 0, "A no complimentary rows");
      check(!conf.customer.includes("Birthday Card"), "A confirmation has no Birthday Card");
      check(!conf.customer.includes("Wishing Card"), "A confirmation has no Wishing Card");
      check(!conf.customer.includes("*Complimentary"), "A confirmation has no complimentary line");
      check(conf.emailRequested === true, "A email-copy requested");
      check(conf.includeReceipt === false, "A include_receipt stays false");
      check(!conf.customer.includes("*Include RECEIPT"), "A customer confirmation has no *Include RECEIPT");
    }
      ...
      paidAddons: [
        {
          code: "birthday_card",
          quantity: 1,
          messages: ["Happy birthday from WB-FIX-ADDON."],
        },
      ],
      emailReceipt: false,
    });
    check(
      birthday.error == null,
      "B Birthday Card submit",
      birthday.error ?? undefined,
    );
    if (birthday.id) {
      const loaded = await loadOrder(birthday.id);
      const conf = confirmationFor(loaded);
      check(conf.amountDue === 138, "B total RM138", String(conf.amountDue));
      check(
        loaded.addons.some((row) => row.code === "birthday_card" && Number(row.unit_price) === 3),
        "B Birthday Card persists at RM3",
      );
      const msg = JSON.stringify(loaded.addons);
      check(msg.includes("Happy birthday from WB-FIX-ADDON."), "B Birthday Card message persists");
      check(conf.customer.includes("~ Birthday Card x1"), "B confirmation Birthday Card x1");
      check(conf.customer.includes("RM3(BC)"), "B confirmation RM3(BC)");
    }
      ...
      paidAddons: [
        {
          code: "wishing_card",
          quantity: 1,
          messages: ["Wishing you a wonderful year ahead."],
        },
      ],
      emailReceipt: false,
    });
    check(wishing.error == null, "C Wishing Card submit", wishing.error ?? undefined);
    if (wishing.id) {
      const loaded = await loadOrder(wishing.id);
      const conf = confirmationFor(loaded);
      check(conf.amountDue === 138, "C total RM138", String(conf.amountDue));
      check(
        loaded.addons.some((row) => row.code === "wishing_card" && Number(row.unit_price) === 3),
        "C Wishing Card persists at RM3",
      );
      check(
        JSON.stringify(loaded.addons).includes("Wishing you a wonderful year ahead."),
        "C Wishing Card message persists",
      );
      check(conf.customer.includes("~ Wishing Card x1"), "C confirmation Wishing Card x1");
      check(conf.customer.includes("RM3(WC)"), "C confirmation RM3(WC)");
    }
      ...
      paidAddons: [
        {
          code: "birthday_card",
          quantity: 1,
          messages: ["Birthday both-card message."],
        },
        {
          code: "wishing_card",
          quantity: 1,
          messages: ["Wishing both-card message."],
        },
      ],
      emailReceipt: true,
    });
    check(both.error == null, "D both cards submit", both.error ?? undefined);
    if (both.id) {
      const loaded = await loadOrder(both.id);
      const conf = confirmationFor(loaded);
      check(conf.amountDue === 141, "D total RM141", String(conf.amountDue));
      check(
        loaded.addons.filter((row) =>
          ["birthday_card", "wishing_card"].includes(String(row.code)),
        ).length === 2,
        "D both cards persist",
      );
      const blob = JSON.stringify(loaded.addons);
      check(blob.includes("Birthday both-card message."), "D birthday message persists");
      check(blob.includes("Wishing both-card message."), "D wishing message persists");
      check(conf.customer.includes("~ Birthday Card x1"), "D confirmation Birthday Card x1");
      check(conf.customer.includes("~ Wishing Card x1"), "D confirmation Wishing Card x1");
      check(
        conf.customer.includes("RM135+RM3(BC)+RM3(WC)= RM141"),
        "D confirmation RM3+RM3 pricing",
        conf.customer.includes("RM3(BC)") && conf.customer.includes("RM3(WC)")
          ? undefined
          : conf.customer,
      );
      const names = loaded.comps.map((row) => String(row.name)).sort().join(", ");
      check(
        /Birthday Topper/.test(names) && /Candle/.test(names) && /Knife/.test(names),
        "E complimentary items persist",
        names,
      );
      check(
        ...
        "E complimentary confirmation line",
      );
      check(conf.emailRequested === true, "email-copy remains a separate column");
      check(conf.includeReceipt === false, "physical include_receipt not set by website submit");
      check(
        !conf.customer.includes("*Include RECEIPT"),
        "customer confirmation does not render *Include RECEIPT from email-copy",
      );
    }
    ...
    check(leftovers.length === 0, "zero leftover SIG/phone rows", String(leftovers.length));
  }

  const failed = checks.filter((row) => !row.ok);
  if (failed.length > 0) {
    console.error(`FAIL guest preorder addon-ambiguity live (${failed.length})`);
    process.exit(1);
  }
  console.log("PASS guest preorder addon-ambiguity live");
}
