...
assert.match(currentCollectionSrc, /lg:w-\[calc\(\(100%-3\.75rem\)\/6\)\]/);
assert.match(currentCollectionSrc, /max-lg:hidden/);
assert.doesNotMatch(currentCollectionSrc, /lg:w-\[12rem\]/);
...
assert.match(extraSrc, /freshPickCustomerStatusLabel/);
...
assert.match(extraSrc, /Limited quantities, available for pickup, dine-in, or delivery/);
...
assert.match(extraSrc, /FRESH_PICKS_ADD_TO_CART_CTA/);
...
assert.doesNotMatch(checkoutPageSrc, /listOrderableMonthlyCatalogues/);
assert.doesNotMatch(checkoutPageSrc, /listClosedPickupOrderDates/);
assert.doesNotMatch(checkoutPageSrc, /loadOperatingHoursSnapshot/);
...
assert.match(actionsSrc, /loadCheckoutCalendarContext/);
assert.match(actionsSrc, /latestOrderableCataloguePickupEnd/);
assert.match(actionsSrc, /entrySpecialUnavailableDates/);
